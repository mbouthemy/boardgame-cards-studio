...
const moods = [
  { name: "Dark", description: "Brooding, restrained, and ominous." },
  { name: "Berserk", description: "Raw steel, brutal action, and high contrast." },
  { name: "Classic", description: "Clear silhouettes, timeless fantasy, and readable detail." },
  { name: "Dreamy", description: "Soft light, surreal colour, and atmospheric worlds." },
  { name: "Horror", description: "Unease, shadows, and unsettling creatures." },
];
...
const csvTemplateRows = [
  ...
  ["03", "Turtle", "Defensive warrior with a hard shell", "Turtle armored warrior, fantasy illustration, detailed, epic, cinematic lighting, 4k"],
  //["04", "Butterfly", "Brew a helpful gust of wind.", "Dark butterfly with glowing wings, fantasy illustration, detailed, epic, cinematic lighting, 4k"],
  // ["05", "Rat", "A gentle guardian blocks the river ford.", "Rat with a wise look, fantasy illustration, detailed, epic, cinematic lighting, 4k"],
];
...
export default function Home() {
  ...
  const [imageProvider, setImageProvider] = useState<"openai" | "gemini" | "cloudflare">("openai");
  ...
  async function startGeneration() {
    if (!project) return;
    try { setSaving(true); setGenerationJob(await api<GenerationJob>(`/api/projects/${project.id}/generation-jobs`, { method: "POST", body: JSON.stringify({ provider: imageProvider }) })); setMessage("Artwork generation completed."); }
  ...
  return <main className="min-h-screen">
    ...
    {view === "projects" ? <>
      <section className="hero"><div className="hero-copy"><p className="eyebrow">BOARD GAME CARD PROTOTYPING</p><h1>Build and download<br /><em>your card game fast.</em></h1><p className="lede">Turn a card list into a styled, illustrated prototype you can review and download in one focused workflow.</p><button className="primary" disabled={saving} onClick={() => void createProject()}>Start a new project <b>-&gt;</b></button></div><div className="hero-art" aria-hidden="true"><div className="hero-card hero-card-back" /><div className="hero-card hero-card-mid"><span>QUEST</span><b>Iron Gate</b></div><div className="hero-card hero-card-front"><span>CREATURE</span><b>Night Warden</b><i>5</i></div><div className="hero-token token-one" /><div className="hero-token token-two" /></div></section>
