...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const { rows } = await db.query("SELECT id, status, model, error_message, created_at, started_at, completed_at FROM generation_jobs WHERE project_id = $1 AND requester_id = $2 ORDER BY created_at DESC LIMIT 1", [params.projectId, userId]);
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const { rows } = await db.query("INSERT INTO generation_jobs (project_id, requester_id, model) VALUES ($1, $2, $3) RETURNING id", [params.projectId, userId, process.env.OPENAI_IMAGE_MODEL || "gpt-image-2"]);
    await runGenerationJob(rows[0].id);
    const { rows: jobRows } = await db.query("SELECT id, status, model, error_message, created_at, started_at, completed_at FROM generation_jobs WHERE id = $1", [rows[0].id]);
    const { rows: results } = await db.query("SELECT gr.card_id, gr.status, gr.storage_key, gr.error_message, c.title FROM generation_results gr JOIN cards c ON c.id = gr.card_id WHERE gr.job_id = $1 ORDER BY c.position", [rows[0].id]);
